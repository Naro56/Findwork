...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($description) || empty($type) || 
        empty($location) || empty($salary) || empty($experience)) {
        $error = 'Please fill in all fields';
    } else {
        // Insert job
        $stmt = $conn->prepare("
            INSERT INTO jobs (title, description, type, location, salary, experience_required, company_id)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
        
        $stmt->bind_param("ssssssi", 
            $title, $description, $type, $location, $salary, 
            $experience, $_SESSION['user_id']
        );
